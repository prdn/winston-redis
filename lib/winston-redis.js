...
Redis.prototype.log = function (level, msg, meta, callback) {
  ...
  this.redis.llen(container, function (err, len) {
    ...
    self.redis.lpush(container, output, function (err) {
      ...
      self.redis.ltrim(container, 0, self.length, function (err) {
        if (err) {
          if (callback) callback(err, false);
          return self.emit('error', err);
        }
...
Redis.prototype.stream = function (options) {
  ...
  if (!this.subscription) {
    this.subscription = redis.createClient(
      this.redis.connectionOption.port,
      this.redis.connectionOption.host
    );
    if (this.redis.auth_pass) {
      this.subscription.auth(this.redis.auth_pass); 
    }
  ...
  this.redis.lrange(container, start, -1, function (err, results) {
    if (stream.destroyed) return;
    ...
    results.reverse().forEach(function (log) {
      try {
